...
class UsersDataFrame():
    ...
    def __init__(self,
        ...
        self.keys = __keys
        self.log = self.__set_raw_data()
        self.users = self.__set_users()
        self.dataframe = self.__set_users_dataframe()
    ...
    def get_users(self):
        return self.dataframe["user"].unique()
    ...
    def get_exploded_user(self, users = None):
        ''' Gets a user as multiple rows using pd.Dataframe.explode '''
        if users != None:
            columns = self.get_keys()[2:]
            df = self.dataframe.loc[self.dataframe['user'].isin(users)].explode(columns)
        return df

    def __set_raw_data(self):
        return pd.read_csv(self.filename, names=self.columns)

    def __set_users(self):
        users = self.log["user"].unique()
        return users
    ...
    def __set_users_dataframe(self):
        __user_data = []
